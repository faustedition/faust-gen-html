...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- allgemeine Entsprechungen zu textTranscript_fuer_Drucke.
	Das Ganze sollte vielleicht separat für beide Pipelines gemeinsam abgelegt werden. -->
	<!-- app und children -->
	...
	<xsl:template match="abbr">
		<span class="appnote" title="Abkürzung">
			<xsl:apply-templates/>
		</span>
	</xsl:template>
	...
	<xsl:template match="add[not(parent::subst)]">
		<xsl:call-template name="enclose">
			...
			<xsl:with-param name="post">
				<span class="app"> erg</span>⟩</xsl:with-param>
	...
	<xsl:template match="subst">
		<span class="appnote"
			title="{concat('»', normalize-space(string-join(del, '')), '« durch »', normalize-space(string-join(add, '')), '« ersetzt')}">
	...
	-->
	<xsl:template
		match="subst[del/subst][count(del/child::node()) = 1 and not(normalize-space(string-join(del/child::text(), '')))]">
		<span class="appnote"
			title="{concat('»', normalize-space(string-join(del/subst/del, '')), 
	...
	<xsl:template match="*">
		<xsl:element name="{f:html-tag-name(.)}">
			...
			<xsl:attribute name="class"
				select="
					string-join((f:generic-classes(.),
					if (@xml:id and key('alt', @xml:id)) then
						'alt'
					else
						(),
					if (@n and @part) then
					...
					<div class="main-content-container">
						<div id="main-content" class="main-content">
							<div id="main" class="print">
								...
								<!-- 1. Spalte (1/5) bleibt erstmal frei -->
								<div class="print-center-column">
									<!-- 2. Spalte (3/5) für den Inhalt -->
									<xsl:apply-templates/>
								</div>
							</div>
						</div>
