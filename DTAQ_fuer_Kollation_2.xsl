...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="text()">
    ...
    <!--<xsl:message><xsl:value-of select="(name(parent::*),$first,position(),.)" separator="/"/></xsl:message>-->
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:if test="not($first) and          matches(.,'^\s') and          normalize-space()!=''">
        ...
        <xsl:choose>
          ...
          <xsl:when
            test="$first and not (following-sibling::text())
			  and string-length()!=0 and
			  normalize-space()=''">
            <xsl:call-template name="space"/>
          </xsl:when>
          <!-- node isn't last, isn't first, and has trailing space -->
          <xsl:when test="not($first) and following-sibling::text()
			  and matches(.,'\s$')">
            <xsl:call-template name="space"/>
          </xsl:when>
          <!-- node isn't last, is first, has trailing space, and has non-space content   -->
          <xsl:when
            test="$first and following-sibling::text() and
			  matches(.,'\s$') and normalize-space()!=''">
            <xsl:call-template name="space"/>
          </xsl:when>
        </xsl:choose>
      </xsl:otherwise>
